...
struct Cli {
    /// Directory from which to load teams, repos and people.
...
async fn run() -> Result<(), Error> {
    ...
    match cli.opts {
        ...
        RootOpts::DumpPermission { ref name } => {
        ...
        RootOpts::DumpIndividualAccess { group_by } => {
            // user -> (repo, access)
